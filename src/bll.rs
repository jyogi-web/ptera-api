...
pub(crate) fn rate_calculation(win: u64, lose: u64) -> (u64, u64) {
    ...
    let (update_win, update_lose) = if win >= lose {
        // 格上が勝った場合
        let delta_win = (((lose + 1.).log2() + 50.0) as u64).min(CONFIG.max_delta_rate);
        let delta_lose = (((win + 1.).log2() + 20.0) as u64).min(CONFIG.max_delta_rate);
        ...
        // 格下が勝った場合
        let delta_win = (((lose + 1.).powi(4).log2() + ((win - lose).abs() + 1.).log2() + 50.0)
            as u64)
            .min(CONFIG.max_delta_rate);
        let delta_lose = (((win + 1.).powi(2).log2() + ((win - lose).abs() + 1.).log2() + 20.0)
            as u64)
            .min(CONFIG.max_delta_rate);
